...
public class GameScreen implements Screen {
    ...
    private Texture spriteSheetTexture = new Texture(
            Gdx.files.internal("spritesheet.png"));
    private Texture background         = new Texture(
            Gdx.files.internal("background.png"));      // Make texture regions
    private Texture backgroundScroll   = new Texture(
            Gdx.files.internal("backgroundscroll.png"));
    ...
    private boolean   firstHighScore  = false;
    public static int cameraFallSpeed = 20;

    private Stage         pauseStage, pauseMenuStage, playMenuStage,
            highScoreStage, userDetailStage, twitterStage;
    private Table         pauseTable, pauseMenuTable, playMenuTable,
            highScoreTable, userDetailTable, twitterTable;
    private Button        okButton, pauseButton, backButton, highScoreButton,
            playButton, userDetailsButton, resumeButton, noThanks,
            twitterButton, rateButton;
    ...
    private TextureRegion   sun         = new TextureRegion(spriteSheetTexture,
            0.666F, 0, 1F, 0.62352941177F);
    ...
    public GameScreen(final BlockJump game, AdRequestHandler androidHandler) {
        ...
        spriteSheet[1] = new TextureRegion(spriteSheetTexture, 100, 0, 100,
                106); // BlockAwake

        player = new Player(
                new TextureRegion(spriteSheetTexture, 0.42666666666F,
                        0.62352941177F, 0.63999999999F, 1F),
                new TextureRegion(spriteSheetTexture, 0, 0.62352941177F,
                        0.2133333333333F, 1F),
                new TextureRegion(spriteSheetTexture, 0.2133333333333F,
                        0.62352941177F, 0.42666666666F, 1F));
        ...
        fit = new FitViewport(Constants.SCREEN_WIDTH, Constants.SCREEN_HEIGHT,
                camera);
        ...
        buttons[0] = new TextureRegion(buttonTexture, 0, 0, 0.166666666667F,
                1F); // Play
        buttons[1] = new TextureRegion(buttonTexture, 0.166666666667F, 0,
                0.333333333334F, 1F); // Highscore
        buttons[2] = new TextureRegion(buttonTexture, 0.333333333334F, 0,
                0.500000000001F, 1F); // back button
        buttons[3] = new TextureRegion(buttonTexture, 0.500000000001F, 0,
                0.666666666668F, 1F); // okbutton
        buttons[4] = new TextureRegion(buttonTexture, 0.666666666668F, 0,
                0.833333333335F, 1F); // detailsbutton
        buttons[5] = new TextureRegion(buttonTexture, 0.833333333335F, 0, 1F,
                1F); // twitter button
        ...
        if (!prefs.contains("name")) {
            ...
            byte[] b = Base64.decodeBase64(
                    prefs.getString("a8dsk4kdgashcas").getBytes());
    ...
    public void render(float delta) {
        ...
        if (GameState.state == GameState.DEAD) {
            fontWidth = game.font.getBounds("Endscore:"
                    + Long.toString(player.getCurrentScore())).width;
            game.font.draw(game.batch,
                    "Endscore:" + Long.toString(player.getCurrentScore()),
                    camera.viewportWidth / 2F - fontWidth / 2F,
                    (float) (camera.viewportHeight * 0.75));
            fontWidth = game.font.getBounds(
                    "Hiscore:" + Long.toString(player.getHighScore())).width;
            game.font.draw(game.batch,
                    "Hiscore:" + Long.toString(player.getHighScore()),
                    camera.viewportWidth / 2F - fontWidth / 2F,
                    (float) (camera.viewportHeight * 0.70));
        } else if (GameState.state == GameState.PLAY_MENU
            ...
            case RUNNING:
                ...
                if (player.isDead()) {
                    if (player.getCurrentScore() > player.getHighScore()) {
                        ...
                        byte[] base64 = null;
                        try {
                            base64 = Base64.encodeBase64(
                                    Long.toString(player.getCurrentScore())
                                            .getBytes("UTF-8"));
                        } catch (UnsupportedEncodingException e) {
                            // TODO Auto-generated catch block
                            e.printStackTrace();
        ...
        // pauseTable.drawDebug(shapeRenderer); // This is optional, but enables
        // debug lines for tables.
    }

    public void renderBackground() {
        int currentY = (int) (GameState.state == GameState.RUNNING
                ? player.getY() : camera.position.y);
        if (currentY < 300) {
            game.batch.draw(background, 0, 0);
            game.batch.draw(backgroundScroll,
                    backgroundX - Constants.SCREEN_WIDTH, 400);
            game.batch.draw(backgroundScroll, backgroundX, 400);
        } else if (currentY > currentScroll - 1000) {
            game.batch.draw(backgroundScroll,
                    backgroundX - Constants.SCREEN_WIDTH, currentScroll - 2000);
            game.batch.draw(backgroundScroll, backgroundX,
                    currentScroll - 2000);

            game.batch.draw(backgroundScroll,
                    backgroundX - Constants.SCREEN_WIDTH, currentScroll);
            game.batch.draw(backgroundScroll, backgroundX, currentScroll);
            if (nextScroll - 1000 < currentY) {
                game.batch.draw(backgroundScroll,
                        backgroundX - Constants.SCREEN_WIDTH, nextScroll);
            ...
            game.batch.draw(backgroundScroll,
                    backgroundX - Constants.SCREEN_WIDTH, 1700);
    ...
    public void renderGame() {
        ...
        for (Block blz : blockLogic.getBlocksMoving()) {
            game.batch.draw(blz.getBlockCurrent(), blz.getBlockRectangle().x,
                    blz.getBlockRectangle().y,
                    blz.getBlockRectangle().getWidth(),
                    blz.getBlockRectangle().getHeight());
        }
        ...
        if (GameState.state == GameState.DEAD_SETUP
                || GameState.state == GameState.DEAD) {
            for (Block blz : blockLogic.getBlocksUnderLava()) {
                game.batch.draw(blz.getBlockCurrent(),
                        blz.getBlockRectangle().x, blz.getBlockRectangle().y,
                        blz.getBlockRectangle().getWidth(),
                        blz.getBlockRectangle().getHeight());
            }
        }

        game.batch.draw(player.getCurrentTexture(), player.getX(),
                player.getY());
        if (64 + player.getX() > Constants.SCREEN_WIDTH) {
            game.batch.draw(player.getCurrentTexture(),
                    player.getX() - Constants.SCREEN_WIDTH, player.getY());
        } else if (player.getX() < 0) {
            game.batch.draw(player.getCurrentTexture(),
                    player.getX() + Constants.SCREEN_WIDTH, player.getY());
        }
        ...
        if (debug == true) {
            game.font.draw(game.batch,
                    Integer.toString(Gdx.graphics.getFramesPerSecond())
                            + " FPS : "
                            + Float.toString(Math.round(
                                    Gdx.input.getAccelerometerX() * 100) / 100)
                    + "X Tilt\n X: " + player.getX() + "\n Y: " + player.getY()
                    + " Highscore: " + Long.toString(player.getCurrentScore())
                    + "/" + Long.toString(player.getHighScore()), 0,
                    textHeight);
        } else if (GameState.state == GameState.RUNNING) {
            game.font.draw(game.batch, Long.toString(player.getCurrentScore()),
                    0, textHeight);
        }
    ...
    public void renderShapes() {
        ...
        float height = GameState.state == GameState.RUNNING ? 800
                : player.getY() + 500;
        shapeRenderer.rect(0, lava.getHeight() - height, Constants.SCREEN_WIDTH,
                height);
    ...
    public void createPauseButton() {
        ...
        style.up = new TextureRegionDrawable(
                new TextureRegion(text, 0, 0, 1F, 1F));
    ...
    public void createUserInput() {
        ...
        if (firstHighScore == false) {
            userDetailTable.row();
            userDetailTable.add("-- HighScores Information --").colspan(2);
            userDetailTable.row();
            userDetailTable.add("Submit for bi-weekly prizes!").colspan(2);
            userDetailTable.row();
            userDetailTable.add("Click the twitter button for more info!")
                    .colspan(2).padBottom(20);
        } else {
            userDetailTable.row();
            userDetailTable.add("You made a highscore!").colspan(2);
            userDetailTable.row();
            userDetailTable.add("Please input details for prizes!").colspan(2);
            userDetailTable.row();
            userDetailTable.add("Note: Email is for contact only.").colspan(2)
                    .padBottom(20);
        }
        ...
        okButton.addListener(new ChangeListener() {
            ...
            public void changed(ChangeEvent event, Actor actor) {
                Preferences prefs = Gdx.app.getPreferences("Blockjump");
                name = nameField.getText().equals("Username") ? "No Name"
                        : nameField.getText();
                email = emailField.getText().equals("Email") ? "No Email"
                        : emailField.getText();
    ...
    public void createHighScores() {
        ...
        // highScoreTable.setBackground(new TextureRegionDrawable(new
        // TextureRegion(new Texture(pm))));
        Skin skin = new Skin(Gdx.files.internal("data/uiskin.json"));
        skin.getFont("default-font").setScale(0.6F);
